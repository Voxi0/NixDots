...
## Basics
<Steps>
  1. **WiFi**: Network manager is used to handle connections. Enable WiFi with `sudo nmcli radio wifi on`
    and connect to your network with `sudo nmcli dev wifi connect <SSID> password <password>`
	2. **Download NixDots (Fresh Install Only)**: If you did a fresh install of NixOS with NixDots, you've noticed we
		...
		```
	3. **Update the system**: [Nix Helper CLI](https://github.com/nix-community/nh) is installed by default.
	...
	4. **Set Timezone**: The timezone will default to UTC so set the correct timezone for you with the command
	...
	5. **Gaming**: Run `flatpak update` to update [Sober](https://sober.vinegarhq.org/) which is the Roblox client I'm
